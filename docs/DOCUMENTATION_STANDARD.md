...
This is the canonical specification for all documentation in the `C:\ToolSnap` project. It contains the full spec — templates, directory layout, migration procedure, rationale, and edge cases.
...
5. **Is it cross-cutting governance?** -> PROJECT-LEVEL. If it's a phase-specific engineering rule, it lives in `docs/rules/` as a Tier 2 doc (see ER S9). Otherwise, its filename must be on the `docs/` top-level whitelist.
...
+-- docs/                                 # project-wide governance and reference
+-- toolsnap/                             # Android app (Kotlin)
+-- toolsnap_db/                          # PC database (Python/PySide6)
...
```
{component}/                    # toolsnap_db or toolsnap
+-- README.md                   # required — what it is, how to run
+-- ARCHITECTURE.md             # required if >5 interacting modules
+-- CHANGELOG.md                # required once version.py exists
+-- version.py                  # authoritative version (single source of truth)
+-- smoke_test.py               # see docs/rules/WRITING_TESTS.md
+-- {module files}
```

No `docs/` subfolder inside a component. Ever. No `STATE.md`. No `VERSION` file.
...
- **Content:** free-form. Open questions, design sketches, intermediate decisions, blockers.
- **Deleted** when the work ships. Shipped content moves to CHANGELOG; structural changes move to ARCHITECTURE; decision rationale moves to an ADR.
- **Staleness:** a `todo/` file older than 30 days with no matching recent delivery is likely abandoned. On contact, either revive it or delete it.

---

## Hard Rules

These are enforced by `validate_docs.py` at delivery time. Each rule cites its validator rule ID.

1. **No version numbers in `.md` filenames** (DOC004). Version lives in `version.py` and CHANGELOG entries. Exception: ADR files use a leading number for ordering.
...
4. **No `docs/` subfolder inside a component** (DOC008). Component docs live at the component root. Project-wide docs live in `C:\ToolSnap\docs\`.
...
Only this `.md` file may live at `C:\ToolSnap\`:
...
|-----------|-------------|----------|
| `rules/` | ER S9 pointer table | Tier 2 engineering rule docs (PROJECT-LEVEL). |
...
Ships with every point release. Module map line counts change, new flows or gotchas get added. Gated by the Pre-Delivery Checklist (ER S6).
...
Use Keep a Changelog structure. Newest entry at the top. Validator enforces prefix format (DOC011) and requires a dated entry matching the bumped version (DOC012).
...
Migration is on-contact, not bulk. The cleanup script has already archived most legacy files to `C:\toolsnap_archive`. When working on ToolSnap in future sessions, review archived files for content worth pulling back into the governed structure:

- Archived architecture content -> `toolsnap_db/ARCHITECTURE.md`
- Archived data model content -> `docs/reference/data_model.md` or ADR
- Archived changelist content -> seed for `toolsnap_db/CHANGELOG.md`
- Archived sync guide content -> `docs/guides/tablet_sync.md`
...
## Existing files to relocate

The following file currently lives in `docs/` but is not a governed doc:

- `docs/setup_toolsnap_structure.ps1` — move to `toolsnap_db/scripts/` or `toolsnap/scripts/` depending on which component it serves. Scripts are not docs.
...
2. **Engineering Rules S5 Documentation Standard** — the summary. Informs Claude's pre-delivery decisions.
3. **Engineering Rules Pre-Delivery Sequence (S6)** — the self-audit hook. Claude walks it before shipping.
