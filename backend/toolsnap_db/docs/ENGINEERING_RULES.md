...
Phase-specific rules live in **Tier 2** at `docs/rules/*.md`. Tier 2 files load on demand, not every session, so the context window stays usable for code. See §9 for the pointer table.
...
2. Check the PK freshness signal (see §10). If stale, say so before writing code.
...
Limit diffs to the smallest surface area that completes the task. Do not refactor, rename, or restructure code that wasn't part of the request. If you notice a nearby issue worth fixing, flag it separately — do not quietly expand the delivery. Scope creep is a Red Flag (§4).
...
3. **"I'm modifying a file I haven't seen the current version of"** — context gap. For first edits, PK is authoritative. For re-edits after a delivery, the file must be re-uploaded. See §2 Context freshness.
4. **"The request conflicts with an ER rule"** — triggers the conflict protocol (§8).
...
6. **"I'm about to use a bare `except: pass`"** — silent exception swallowing. See §5.
...
Rules that apply across phases. Phase-specific rules live in Tier 2 (§9).
...
- **Never use bare `except: pass`** — silent exception swallowing is a Red Flag (§4). Catch specific exception types; log and either recover explicitly or re-raise.
...
- **No `docs/` subfolder inside a component.** Component docs live at the component root; project-wide docs live in `C:\ToolSnap\toolsnap_db\docs\`.
...
4. **Test review** — answer the three questions from `docs/rules/WRITING_TESTS.md` §Pre-delivery: what was executed, what could have been but wasn't, what can't be tested here. Untestable items → TESTING_NOTES in CHANGELOG.
...
- **Naming:** `ts_{project}_{version}.zip` or `ts_{project}_{version}_{label}.zip`.
- **Paths inside the zip are project-relative**, rooted at `C:\ToolSnap\toolsnap_db\`. Example: `core/importer.py`. Uncertain destinations go to zip root — DRH drops them in the project root for manual placement.
...
Example: *"Note: this conflicts with ER §5 line limit (2000 default maximum). Proceeding per your instruction."* — then code.
...
If the request is fully covered by Tier 1 rules alone (a quick single-line fix, a naming question answered by §5), say so and proceed without loading Tier 2.
...
4. **Classify the phase.** See §9. Ask for the Tier 2 files that apply, with a one-line reason per file.
