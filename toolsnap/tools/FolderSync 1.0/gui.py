...
class FolderSyncWindow(QMainWindow):
    ...
    def _on_find_custom(self):
        """Handle Find Other button - search for folders with custom name."""
        ...
        folder_name, ok = QInputDialog.getText(
            self,
            "Find Folders",
            "Enter folder name to search for:",
            text="tools"
        )
        
        if ok and folder_name:
            self._search_and_add_folders(folder_name)
    
    def _search_and_add_folders(self, folder_name: str):
        """
        ...
        """
        # Show search dialog
        dialog = FolderSearchDialog(folder_name, self)
