# collect_toolsnap_clean.ps1
# Collects a clean zip of C:\toolsnap, excluding build artifacts, caches, and junk.
# Output: Desktop\toolsnap_clean_<timestamp>.zip

$ProjectRoot = [System.IO.Path]::GetFullPath("C:\toolsnap").TrimEnd('\', '/')
$ProjectName = "toolsnap"
...
Write-Host "  Root (normalized): $ProjectRoot" -ForegroundColor DarkGray
...
try {
    Get-ChildItem -Path $ProjectRoot -Recurse -File | ForEach-Object {
        ...
        if ($normalizedFull.Length -le $ProjectRoot.Length) {
            $filesSkipped++
            return
        }
        $relativePath = $normalizedFull.Substring($ProjectRoot.Length).TrimStart('\', '/')
