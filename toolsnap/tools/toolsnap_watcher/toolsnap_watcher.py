"""
Downloads Watcher for ToolSnap Android Project
Watches ~/Downloads for ts_* files and routes them into C:\\toolsnap.

ZIP files with recognized structure (app/, tools/, docs/, gradle/) are
auto-extracted to the correct paths — zero popups.

Single files with a pattern match are moved silently.
Unmatched files are logged and skipped.

Run: python toolsnap_watcher.py
"""
...
WATCHER_DIR = TOOLSNAP_ROOT / "tools" / "toolsnap_watcher"
...
KNOWN_ROOT_FOLDERS = {'app', 'gradle', 'tools', 'docs'}
...
def get_suggested_destination(filename):
    ...
    # Regex patterns from watcher_routes.json
...
def detect_zip_mode(zip_contents):
    ...
    # Direct match — zip roots ARE toolsnap folders
    ...
    if len(root_folders) == 1:
        ...
        if nested & KNOWN_ROOT_FOLDERS:
            # Recover actual case of wrapper
            for path in zip_contents:
                parts = path.replace('\\', '/').split('/')
...
def get_structure_destination(zip_path, strip_wrapper=None):
    ...
    full_path = TOOLSNAP_ROOT / dir_path
...
# ============================================================================
# ZIP EXTRACTION (structure mode — the magic path)
# ============================================================================
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            for entry in contents:
                ...
                if dest_dir in ('', 'ROOT'):
                    dest_path = TOOLSNAP_ROOT
                elif is_flagged:
                    dest_path = DOWNLOADS_DIR
                    flagged.append(filename)
                else:
                    dest_path = TOOLSNAP_ROOT / dest_dir
                ...
                # Extract to temp then move (avoids nested dir issues)
                temp_dir = TOOLSNAP_ROOT / '__temp_extract__'
                temp_file = Path(zf.extract(entry, temp_dir))
                shutil.move(str(temp_file), str(final))
                extracted += 1

                tag = "⚠️ →Downloads" if is_flagged else f"→ {dest_dir or 'ROOT'}"
        ...
        zip_path.unlink()
        print(f"  📦 {zip_path.name}: {extracted} files extracted, zip deleted")
...
# ============================================================================
# ZIP EXTRACTION (flat mode — dump all to one spot)
# ============================================================================
...
def extract_zip_flat(zip_path, dest_path):
    ...
    try:
        ...
        zip_path.unlink()
        print(f"  📦 {zip_path.name}: {count} files → {dest_path}, zip deleted")
    except Exception as e:
        print(f"  ERROR extracting {zip_path.name}: {e}")
...
class ZipTreeDialog:
    ...
    with Process / Skip buttons.  Used at startup and during live watching.
    """
    ...
    def _build(self):
        self.root = tk.Tk()
        self.root.title(f"📦 {self.zip_path.name}")
        ...
        # Header
        ttk.Label(main, text=f"📦 {self.zip_path.name}",
    ...
    def _populate_tree(self):
        """Build folder/file hierarchy in treeview."""
        # Normalize and sort
        ...
        # Track inserted folder nodes  (display_path → tree item id)
        folder_ids = {}
...
# ============================================================================
# UNMATCHED FILE POPUP (minimal — for single files with no pattern match)
# ============================================================================

class UnmatchedFilePopup:
    """
    Tiny popup for a single file that had no pattern match.
    Shows filename, lets user Ignore Once / Always Ignore.
    """
...
class DownloadsHandler(FileSystemEventHandler):
    ...
    def _handle_zip(self, filepath):
        ...
        if mode == 'structure':
            print(f"📦 Structure zip: {filepath.name}")

            # Show tree preview and wait for confirmation
            ...
            if flagged:
                ...
                messagebox.showwarning(
                    ...
                    f"(paths not found in toolsnap):\n\n{names}")
        else:
            # Flat zip — just extract to project root
            print(f"📦 Flat zip (no structure): {filepath.name} → project root")
            extract_zip_flat(filepath, TOOLSNAP_ROOT)
...
def scan_existing_files(ignore_list):
    """Find ts_* files already sitting in Downloads."""
    files = []
...
def process_startup(ignore_list):
    ...
    Returns set of filenames shown (so live watcher can skip them).
    """
    files = scan_existing_files(ignore_list)
    if not files:
        print("No existing ts_* files to process.")
        return set()
    ...
    for filepath in files:
        ...
        if is_zip:
            ...
            if mode == 'structure' and contents:
                ...
                # Flat zip — extract to root
                print(f"📦 Flat zip: {filepath.name} → project root")
                extract_zip_flat(filepath, TOOLSNAP_ROOT)
            else:
                print(f"  ⚠️ Empty zip: {filepath.name}")
        else:
            # Single file — pattern route silently
            dest = get_suggested_destination(filepath.name)
            if dest is not None:
                dest_dir = dest if dest not in ('', 'ROOT') else ''
                dest_path = TOOLSNAP_ROOT / dest_dir if dest_dir else TOOLSNAP_ROOT
                dest_path.mkdir(parents=True, exist_ok=True)
                final = dest_path / filepath.name
                if final.exists():
                    final.unlink()
...
def main():
    print("=" * 60)
    print("  ToolSnap Downloads Watcher")
    print("=" * 60)
    print(f"Watching: {DOWNLOADS_DIR}")
    print(f"Prefix:   {TOOLSNAP_PREFIX}*")
    print(f"Target:   {TOOLSNAP_ROOT}")
    ...
    if not TOOLSNAP_ROOT.exists():
        print(f"ERROR: toolsnap folder not found: {TOOLSNAP_ROOT}")
        return
